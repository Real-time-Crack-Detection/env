...
class CrackManager:
    ...
    def predict(self, img):
        ...
        for jk in range(R.shape[0] // self.C.num_rois + 1):
            ...
            for ii in range(P_cls.shape[1]):
                ...
                bboxes[cls_name].append(
                    [self.C.rpn_stride * x, self.C.rpn_stride * y, self.C.rpn_stride * (x + w),
                     self.C.rpn_stride * (y + h)])
        ...
        candidate = img[min_y1:max_y2, min_x1:max_x2]

        candidate = cv2.resize(candidate, (100, 75))
        candidate = np.expand_dims(candidate, axis=0)
        result = self.classification_model.predict(candidate)
        # classify 수행
...
# c.predict('./testData/9.60310454-concrete-crack-floor-texture.jpg')
# c.predict('./testData/9.DSC08006.jpg')
# c.predict('./testData/23.0702050042-01-Daigonal-Cracks-In-Brick-Walls.jpg')
# c.predict('./testData/34.cracked-concrete-wall1.jpg')
# c.predict('./testData/39.old-damaged-textured-brick-wall-with-vertical-crack-WKYGPG.jpg')
# c.predict('./testData/44.Concrete-Crack-Repair.jpg')
#
